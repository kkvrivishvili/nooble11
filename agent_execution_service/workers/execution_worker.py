...
class ExecutionWorker(BaseWorker):
    ...
    async def _handle_action(self, action: DomainAction) -> Optional[Dict[str, Any]]:
        ...
        Nota: BaseWorker maneja automáticamente los callbacks cuando
        callback_action_type está presente. No es necesario manejarlos aquí.
        """
        try:
            if not self.execution_service:
                raise RuntimeError("ExecutionService no está inicializado")

            # Delegar al servicio - BaseWorker maneja callbacks automáticamente
