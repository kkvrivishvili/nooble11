    ...
    async def _process_action_loop(self):
        ...
        while self._running:
            ...
            try:
                ...
                logger.info(f"[{self.service_name}][{self.consumer_name}] Acción {action.action_id} ({action.action_type}) recibida del stream (MsgID: {message_id_to_ack})", extra=log_extra)
